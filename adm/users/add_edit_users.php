...
if (isset($_POST['add'])) {
    ...
    $qry_check = mysql_query("SELECT username FROM users where username='$user_name' ", $con);
    if (!$qry_check) {
    ...
    $save = mysql_query("INSERT INTO users (id, username, password) VALUES ('$str','$user_name','$randompass')");
...
if (isset($_GET['user_id'])) {
    ...
    $qry1 = mysql_query("SELECT * FROM users WHERE id='$user_id'", $con);
    if (!$qry1) {
        die("Query Failed: " . mysql_error());
    }
...
if (isset($_POST['edit'])) {
	...
	if ($pass != $pass2) { die ('<p style="color: red; font-weight: bold; text-align: center;">Password do not match!<br /> Please try again...</p>'); }
	
    $save = mysql_query("UPDATE users SET username='$user_name', password='$pass' WHERE id='$user_id'", $con);
