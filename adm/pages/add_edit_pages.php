...
if (isset($_POST['add'])) {
    ...
    $qry_check = mysql_query("SELECT title FROM pages where title='$title' ", $con);
    if (!$qry_check) {
    ...
    $save = mysql_query("INSERT INTO pages (img, title, date, link, text) VALUES ('$img','$title',NOW(),'$link','$text')");
...
if (isset($_GET['id'])) {
    $id = $_GET['id'];
    $qry1 = mysql_query("SELECT * FROM pages WHERE id='$id'", $con);
    if (!$qry1) {
        die("Query Failed: " . mysql_error());
    }
...
if (isset($_POST['edit'])) {
    ...
    $save = mysql_query("UPDATE pages SET title='$title', date='$c_date', link='$link', text='$text' WHERE id='$id'", $con);
    if (!$save) {
        die("Query Failed: " . mysql_error());
    }
    ?>
